...
class Config:
    ...
    holidays: Set[date] = field(
        default_factory=set
    )  # Set of dates to exclude from business day calculations
...
def from_env() -> Config:
    ...
    return Config(
        ...
        use_business_days=bool(
            os.environ.get("JIRA_USE_BUSINESS_DAYS", "").lower() in ("1", "true", "yes")
        ),
