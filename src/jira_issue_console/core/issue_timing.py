...
from .issues import _parse_jira_datetime, normalize_status
...
def calculate_status_timing(
    ...
    for i in range(len(transitions) - 1):
        ...
        if use_business_days:
            ...
            if (end.date() > start.date() and 
                end.date().weekday() < 5 and 
                end.date() not in holidays):
                days += 1.0
        else:
    ...
    if use_business_days:
        ...
        if (now.date() > final["date"].date() and 
            now.date().weekday() < 5 and 
            now.date() not in holidays):
            days += 1.0
    else:
