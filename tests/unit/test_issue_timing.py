...
def test_calculate_status_timing_with_workflow():
    ...
    workflow = WorkflowConfig(
        status_map={"In Review": "In Progress", "Testing": "In Progress"}
    )
...
def test_calculate_status_timing_business_days():
    ...
    timing = calculate_status_timing(transitions, use_business_days=True)

    assert timing["Open"] == pytest.approx(
        3.0
    )  # Friday + Monday + Tuesday (up to transition time)
