...
async def test_list_issues_with_custom_jql():
    ...
    payload = {"issues": [{"id": "1", "key": "PROJ-1", "fields": {"summary": "High priority"}}]}
...
async def test_list_issues_empty_jql():
    ...
    # Arrange: mock Jira search endpoint
    url = "https://jira.example.com/rest/api/2/search"
    payload = {"issues": [{"id": "1", "key": "PROJ-1", "fields": {"summary": "Normal issue"}}]}
    route = respx.get(url).respond(200, json=payload)
    ...
    assert route.calls
    assert route.calls[0].request.url.params.get("jql") == "project=PROJ AND status!=Done"
    assert len(res) == 1
    assert res[0]["key"] == "PROJ-1"
